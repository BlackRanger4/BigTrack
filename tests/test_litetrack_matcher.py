...
class LiteTrackMatcherTest(unittest.TestCase):
    ...
    def test_match_uses_candidate_and_stored_template_features(self) -> None:
        backend = _FakeLiteTrackBackend()
        matcher = LiteTrackMatcherModel(config=self.config, backend=backend)
        state = matcher.initialize_template(
            self.frame,
        ...
        self.assertAlmostEqual(evidence.match_score, 0.85, places=5)
    ...
    def test_simple_bigtrack_integration_with_fake_backend(self) -> None:
        ...
        self.assertAlmostEqual(output.confidence, 0.85, places=5)
        self.assertEqual(backend.forward_count, 1)

    def test_real_checkpoint_smoke_when_enabled(self) -> None:
        if os.environ.get("BIGTRACK_RUN_LITETRACK_REAL_SMOKE") != "1":
            raise unittest.SkipTest(
                "set BIGTRACK_RUN_LITETRACK_REAL_SMOKE=1 to run LiteTrack real checkpoint smoke"
            )
